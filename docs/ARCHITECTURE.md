...
**operation**: one op.
- `id`, `date`, `attendance_start` (20:00), `attendance_end` (23:00), `event_end` (23:30), `discord_event_id`, `prepared_at` (mission makers pinged), `announced_at` (@everyone posted), `name`, `source` (`auto_weekly` | `manual`)
...
- **Phase 6 - Attendance:** Operations-channel sampling, session reconstruction, read-only member and site views, guest auto-backfill on link. No historical import.
