...
- **Phase 2 - Identity (minimal web app):** Discord login (Better Auth), member profile, TeamSpeak linking (pick-from-list + poked code, as web pages that serve until Phase 4's `/link` replaces them: ADR 0017), Steam OpenID linking. No public content yet. Import the legacy identity links here.
...
**Cutover is incremental, not big-bang.** The old Discord bot and the old website are both still running, and both keep running. Nothing currently writes TeamSpeak server-groups: the legacy sync was never finished, and the attendance recorder's last write was 2024-07-27. So the new reconcile is the only writer, there is no old sync to fight, and no "kill the old sync" step is needed. The two are separate accounts on separate tokens (ADR 0015), so nothing we deploy can disturb the old bot and it can be retired on its own schedule, with no credential handover and no cutover instant. Its `!` prefix commands cannot collide with our slash commands either. The only collision is inside our own application: `7R_Bot` may still carry `/loa`, and clearing that is ours to do (Phase 4). TeamSpeak groups are maintained **by hand** today, and that hand-work is the toil this project actually removes.
