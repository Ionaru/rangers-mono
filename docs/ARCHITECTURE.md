...
| TeamSpeak link | One at a time, self-service; proven by picking self from the online-clients list and typing back a code the bot pokes, via `/link` in Discord (replaces the web flow when it ships) | ADR 0017 |
...
- **Self-service TeamSpeak linking:** `/link` and `/unlink`, **member-facing, not admin-gated** (the only such write commands). `/link` runs the possession challenge in Discord: an ephemeral select of online unlinked TS clients, the worker pokes the picked client a one-time code, an [Enter code] button opens the modal the code is typed into. Replaces the web link flow (ADR 0017).
...
- **Phase 2 - Identity (built, minimal web app):** Discord login (Better Auth), member profile, TeamSpeak linking (pick-from-list + poked code, as web pages that serve until Phase 5's `/link` replaces them: ADR 0017), Steam OpenID linking. No public content yet. Import the legacy identity links here.
