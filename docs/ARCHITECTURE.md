...
- **Memes:** ported from `fun.py` (the image+caption pairs, the "slav" set, `!ff`/`!monkey`/`!medic`/`!8ball`/`!rps`, the `!slotting` squad-dealer), now slash commands with ephemeral replies and per-role gating. **The images are not hosted by the old bot.** They are Discord CDN attachment links hardcoded in `fun.py`, and they now return **404 to any anonymous client**: since 2023-24 Discord CDN links require signed `ex`/`is`/`hm` params, and they still render in Discord only because the client re-signs its own URLs. Procedure: call `POST /api/v9/attachments/refresh-urls` with the bot token to get freshly-signed URLs, download all 25 images, commit them to the repo, and serve them from our own domain.
...
- **The 2019 bot token was never leaked, and does not need rotating.** `joeyyyb/7r-discordbot` has been private since it was created in 2019; earlier drafts of these docs assumed it had been public and built a rotate-first ordering on top of that. It had not. Reuse the token (Phase 0, §9). What remains true: it is committed to that repo's history, so treat it as a credential that several people can read, and do not commit it to this one.
...
- **Bin:** EOL toolchains, the Nx build, discord.py 1.3.4, the old secret handling (committed into the repo rather than injected), the add-only Discord sync, the empty legacy OperationModel, the 7-permission model.
...
- **Phase 0 - Prep (no code):** harvest and commit the 25 meme images via `/attachments/refresh-urls`; **create the 8 badge roles in Discord and backfill the 83 legacy grants** (scriptable: every legacy user has a Discord id); confirm the reused Discord app / guild / GHCR namespace. The legacy dump is already at `data/Dump20260711.sql`. (No token rotation: §7.)
