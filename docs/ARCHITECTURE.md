...
- **Least privilege** for `7R_Bot`: **`CREATE_EVENTS` + `MANAGE_ROLES`**, plus the **GUILD_MEMBERS** privileged intent enabled on *its* application in the developer portal (required for the REST member list, not only for the gateway). Nothing more. Not `MANAGE_EVENTS`: it cannot create. Today it holds **Administrator**, left over from its `/loa` days. That covers both permissions and everything else besides, so nothing is blocked by it, but the token now lives in a `.env` on a box we do not own (ADR 0005, ADR 0014), and Administrator turns a leaked token into a lost guild. Dialling it back is a Phase 0 task.
