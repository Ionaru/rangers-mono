...
- **Phase 4 - Discord bot:** interactions endpoint, slash memes, role inspection, `/role` and `/rank set`, `/link-force`, and the weekly scheduled-event job (which creates Operations). Two steps here are destructive by design, on our own application and nobody else's: registering commands is a bulk overwrite of `7R_Bot`'s command list, which takes any surviving `/loa` with it (there is no LOA feature: ADR 0010), and pointing the Interactions Endpoint URL at us takes over delivery for every command that application owns.
