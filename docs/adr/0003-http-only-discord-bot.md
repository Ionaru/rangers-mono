...
- **The bot account is `7R_Bot`** (ADR 0015), our own Discord application, already in the guild. `DISCORD_BOT_TOKEN` and `DISCORD_PUBLIC_KEY` both come from it, and the Interactions Endpoint URL is set on it. The key is per-application: a public key from one application and a token from another is precisely the silent, delayed death described above, and it is reachable by a single wrong line in `.env`. That application previously served the legacy `/loa`. The account is adopted; the feature is not (ADR 0010), so any surviving `/loa` registration on it gets deleted.
