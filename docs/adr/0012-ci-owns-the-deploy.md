...
The deploy therefore points `DOCKER_CONFIG` at a directory it writes itself, holding a `config.json` that names no credential helper **and carries a non-empty `auths` map**. No credentials are needed by anything here: the GHCR packages inherit the repository's visibility and are public, and postgres comes from Docker Hub. The operator's own `~/.docker/config.json` is untouched, and a fresh box needs no preparation.

The `auths` map is the part that actually does the work, and it took three failed deploys to find out. **Emptying `credsStore` does not disable the credential helper.** When a config contains no auth of any kind, the docker CLI calls `DetectDefaultStore`, which picks the platform default (`wincred` on Windows) and uses it if the binary is on `PATH`, which Docker Desktop guarantees. So an empty config does not turn the helper off, it swaps `desktop` for `wincred`, and both read the Windows Credential Manager and both die the same way. The CLI skips that detection only when the config already contains auth (`if !configFile.ContainsAuth()`), and a populated `auths` map is what satisfies it. The entries are empty objects: they assert that these registries need no credentials, which is true, and that assertion is what stops the CLI going to look for a helper to ask.
