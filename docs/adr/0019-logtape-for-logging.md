...
Both services log through **`@logtape/logtape`, pinned to 2.3.0**, reached only through a new workspace package, `@7r/logging`, which owns the dependency and the policy. Every module that logs calls `getLogger(["7r", ...])` at module scope; every entry point calls `configureLogging` exactly once. The `log: (message, extra) => void` callback that was threaded through five modules is gone. The `alert` callback is not, and that asymmetry is the point (see below).
...
**The two services do not agree on what a log line is.** The worker had a five-line JSON-lines helper in `main.ts` and passed it down as a parameter. `apps/web` had five bare console calls (four `console.error` and one `console.warn`) with `[web]`/`[discord]` string prefixes and no structure at all. One box, one `docker compose logs`, two formats, and the website's half unparseable.
...
- **Converting the operator CLIs.** 129 of the 137 `console.*` calls this change found were in one-shot scripts (`sync:preview`, `op:preview`, `ts:check`, `badges:backfill`, `assignables:seed`, `env:check`, `phase0:check`, `commands:register`, `import:legacy`, `migrate`). That is a user interface printed to stdout for a human at a terminal, not logging, and a mechanical sweep would have wrecked it. Every line of it survives. Exactly one call was removed from that set and it was not output: `op-preview.ts` had a `log:` callback whose whole body was a `console.error`, a bridge from the shared code's logger to the terminal, and `configureLogging({ shape: "text" })` is what replaces it.

## Consequences

- **One dependency, seven files.** ADR 0006's dual-manifest rule means `@logtape/logtape` lands in `packages/logging/package.json`, the package is listed in the root `deno.json` workspace **and** its `check` task, `apps/web/package.json` gains `@7r/logging`, and `apps/web/deno.prod.json` gains the npm pin. That last one is not optional and not obvious: the build bundles `@7r/logging` like every other workspace package but leaves `@logtape/logtape` itself a bare specifier (npm dependencies are not bundled), so without the entry the runtime image fails at `deno cache`. It sits in a chunk that `entry.mjs` reaches by dynamic import rather than in `entry.mjs` itself, which changes nothing: `deno cache dist/server/entry.mjs` follows it, which is what makes the image's `--cached-only` boot survive an npm outage. A `check @logtape/logtape packages/logging/package.json` line was added to the pin-consistency job in `cd.yaml` so the two pins cannot drift.
